...
# 从 cap.py 中导入 capsule network 网络结构（确保该结构与训练时一致）
from cap import build_network
...
def plot_prediction(index=0,
                    ...
                    model_ckpt_path=r"models/cap_experiment/cap_model.ckpt-70"):
    """
    1) 从 test3.hdf5 中加载测试数据；
    2) 用训练好的 capsule network 对第 index 个样本进行预测；
    3) 分别绘制三个图：
    ...
    with tf.compat.v1.Session() as sess:
        ...
        c1 = ax1.pcolormesh(tauvals, Kvals, trace_2d, cmap='jet')
        ...
        # ------------------- 计算 XUV 复数光谱 -------------------
        # 通过预测的 xuv_coefs_pred 生成预测的 XUV 复数光谱
        ...
        # 通过真实的 xuv_coefs_true 生成真实的 XUV 复数光谱
...
if __name__ == "__main__":
    # 例如，对测试集第 2 个样本进行推断，并绘制相应图像，同时输出该样本的 MSE
