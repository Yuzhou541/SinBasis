...
# 从 multi.py 中导入混合模型网络结构（确保推断时的网络结构与训练时一致）
from multi import build_hybrid_network
import phase_parameters.params as params
import tf_functions
import xuv_spectrum.spectrum    # 若可用
import ir_spectrum.ir_spectrum  # 若可用
...
def plot_prediction(index=95,
                    test_hdf5=r"C:\Users\ROG\Desktop\attosecond_streaking_phase_retrieval-ir_cep_fix_3\test3.hdf5",
                    model_ckpt_path=r"models\hybrid_experiment\hybrid_model.ckpt-70"):
    """
    1) 从 test3.hdf5 中加载测试数据；
    2) 使用训练好的混合模型对第 index 个样本进行预测；
    3) 分别绘制三个图：
         - 图1：原始 streaking trace（二维图）；
         - 图2：预测的 XUV 光谱（强度与相位）；
         - 图3：真实的 XUV 光谱（强度与相位）；
    4) 同时计算并输出该测试样本的均方误差（MSE）。
    """
    # ------------------- 读取 HDF5 测试数据 -------------------
    ...
    # 取出第 index 个样本
    trace_1d = trace_data[index]          # 一维，长度 29498
    xuv_coefs_true = xuv_coefs_data[index]  # 真实 xuv 系数（5 维）
    ir_params_true = ir_params_data[index]  # 真实 ir 参数（4 维）
    ...
    with tf.compat.v1.Session() as sess:
        ...
        pred = sess.run(pred_params, feed_dict=feed)[0]  # 得到 9 维输出
        xuv_coefs_pred = pred[:5]
        ir_params_pred = pred[5:]
        
        # ------------------- 输出测试样本的均方误差 -------------------
        # 将真实的 xuv_coefs 与 ir_params 拼接成目标向量
        ...
        # ------------------- 图1：Streaking Trace -------------------
        # 试图使用 params 中的 delay_values 和 K；如果未定义则使用默认值
        try:
        ...
        # 根据 xuv_f_pred 的长度生成光子能量轴（假设范围 50～350 eV）
        ...
        ax3.plot(photon_energy, amplitude_actual, color='red', linestyle='--', label="Actual Intensity")
        ax3.set_xlabel("Photon Energy [eV]")
        ax3.set_ylabel("Intensity (a.u.)", color="red")
        ax3_twin = ax3.twinx()
        ax3_twin.plot(photon_energy, phase_actual, color='blue', linestyle='--', label="Actual Phase")
        ax3_twin.set_ylabel("Phase [rad]", color="blue")
...
if __name__ == "__main__":
    # 例如，对测试集第 2 个样本进行推断，并分别绘制 streaking trace、预测的 XUV 光谱和真实的 XUV 光谱，
    # 同时输出该样本的均方误差（MSE）
    plot_prediction(index=69,
                    test_hdf5=r"C:\Users\ROG\Desktop\attosecond_streaking_phase_retrieval-ir_cep_fix_3\test3.hdf5",
                    model_ckpt_path=r"models\hybrid_experiment\hybrid_model.ckpt-25")
