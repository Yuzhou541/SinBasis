...
def proof_trace(trace):
    ...
    proof = tf.math.real(tf_ifft(tensor=filtered_f, shift=int(len(phase_parameters.params.delay_values) / 2),
                           axis=1))
...
def tf_ifft(tensor, shift, axis=0):
    ...
    shifted = tf.roll(tensor, shift=shift, axis=axis)
    # fft
    time_domain_not_shifted = tf.signal.ifft(shifted)
    # shift again
    time_domain = tf.roll(time_domain_not_shifted, shift=shift, axis=axis)
...
def tf_fft(tensor, shift, axis=0):
    ...
    shifted = tf.roll(tensor, shift=shift, axis=axis)
    # fft
    freq_domain_not_shifted = tf.signal.fft(shifted)
    # shift again
    freq_domain = tf.roll(freq_domain_not_shifted, shift=shift, axis=axis)
...
def streaking_trace(xuv_cropped_f_in, ir_cropped_f_in):
    ...
    # ------------gather values from integrated array-------------------
    # ------------------------------------------------------------------
    ir_values = tf.gather(A_t_integ_t_phase, delayindexes.astype(int))
    ir_values = tf.expand_dims(tf.expand_dims(ir_values, axis=0), axis=3)
    # for the squared integral
    ir_values_2 = tf.gather(A_t_integ_t_phase_2, delayindexes.astype(int))
